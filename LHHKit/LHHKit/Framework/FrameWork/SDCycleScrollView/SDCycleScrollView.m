 ...
 */
NSString * const SDCycleScrollViewResetPlayerNotifity = @"SDCycleScrollViewResetPlayerNotifity";
...
@property (nonatomic, strong) NSMutableDictionary * imagePathsDic;
...
- (void)initialization
{
    self.imagePathsDic = [NSMutableDictionary dictionary];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(resetPlayer) name:SDCycleScrollViewResetPlayerNotifity object:nil];
}

+ (instancetype)cycleScrollViewWithFrame:(CGRect)frame delegate:(id<SDCycleScrollViewDelegate>)delegate placeholderImage:(UIImage *)placeholderImage isCanVideo:(BOOL)isCanVideo
{
    SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    cycleScrollView.delegate = delegate;
    cycleScrollView.placeholderImage = placeholderImage;
    cycleScrollView.isCanVideo = isCanVideo;
...
- (void)setIsCanVideo:(BOOL)isCanVideo
{
    _isCanVideo = isCanVideo;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString * imagePath = [self.imagePathsGroup[itemIndex] APPImgURL];
    ...
    if (!self.onlyDisplayText && [imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
            ...
            if (self.isCanVideo)
            {
                if ([Validator isVideo:imagePath])
                {
                        [self getVideoPreViewImage:imagePath complete:^(UIImage *image) {
    ...
    @weakify(self);
    @weakify(cell);
    cell.whenTouchVideo = ^(NSString *imagePath) {
        @strongify(self);
        @strongify(cell);
        
        ZFPlayerModel *playerModel = [[ZFPlayerModel alloc] init];
        playerModel.videoURL         = [NSURL URLWithString:cell.imagePath];
        playerModel.scrollView       = collectionView;
...
{
    if ([self.imagePathsDic objectForKey:path] &&
        [[self.imagePathsDic objectForKey:path] isKindOfClass:[UIImage class]])
    {
        complete([self.imagePathsDic objectForKey:path]);
    }
    else
    {    
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            [self.imagePathsDic setObject:videoImage forKey:path];
            complete(videoImage);
        });
    }
}
