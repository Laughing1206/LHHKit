...
{
    if ( failureBlock ) {
        HTTPResponseError * responseError = [[HTTPResponseError alloc] init];
        responseError.code = [NSString stringWithFormat:@"%ld",error.code];
        if (error.code == -1009)
        {
            responseError.message = @"网络不给力，请稍候再试";
        }
        failureBlock(nil, responseError);
    }
}
