...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self setTitle:@"正在加载" forState:MJRefreshHeaderStateRefreshing];
		
		// 背景

		if ( !self.bgImage )
		{
			UIImageView * bgImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"mj_resources.bundle/mj_drop_down_bg"]];
			self.bgImage = bgImage;
			[self addSubview:self.bgImage];
			[self bringSubviewToFront:_stateLabel];
			[self bringSubviewToFront:_updatedTimeLabel];
		}
    }
...
- (void)layoutSubviews
{
	...
	self.bgImage.frame = CGRectMake(0, -185, self.width, self.height + 185);
...
- (void)setState:(MJRefreshHeaderState)state
{
    ...
    // 设置状态文字
    [self setTitle:_stateTitles[@(_state)] forState:_state];
    ...
    switch (state) {
        case MJRefreshHeaderStateIdle: {
            if (oldState == MJRefreshHeaderStateRefreshing) {
                ...
                // 恢复inset和offset
                [UIView animateWithDuration:MJRefreshSlowAnimationDuration delay:0.3 options:UIViewAnimationOptionAllowUserInteraction|UIViewAnimationOptionBeginFromCurrentState animations:^{
                    // 修复top值不断累加
        ...
        case MJRefreshHeaderStateRefreshing: {
            [UIView animateWithDuration:MJRefreshFastAnimationDuration delay:0.0 options:UIViewAnimationOptionAllowUserInteraction|UIViewAnimationOptionBeginFromCurrentState animations:^{
                ...
                if ([self.refreshingTarget respondsToSelector:self.refreshingAction]) {
                    msgSendT(msgTarget(self.refreshingTarget), self.refreshingAction, self);
                }
            }];
            break;
        }
