...
{
    if ([string isNoEmpty] && string.length == 11)
    {
        return YES;
    }
    return NO;
...
{
    NSString *phoneRegex = @"^[0-9]+(\\.[0-9]{1,100})?$";
...
+ (BOOL) isVideo:(NSString *)URL
{
    if (URL &&
        ...
        URL.length)
    {
        for (NSString * videoFormat in [self videoFormat])
        {
            if ([[URL lowercaseString] hasSuffix:videoFormat])
            {
                return YES;
            }
        }
